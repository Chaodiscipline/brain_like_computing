...
# Define constants
lamda = 1.0     # Reconstruction coefficient
...
def main():
    ...
    # Create samples.
    train = pd.read_csv('/storage/beishicheng/deep_learning/brain/Topoica/mnist_train.csv')
    Y_train = train['label']
    X_train = train.drop(labels = ["label"],axis = 1)
#     X_train = np.array(X_train)
#     for i in range(len(X_train)):
#         for j in range(784):
#           X_train[i][j] = X_train[i][j].astype('f')/255.
    X_train = np.array(X_train, dtype='f').reshape((-1,1,28,28))
    validation = X_train[0:100]
    ...
    visualize(validation, 'validation.png', (28, 28))
    ...
    for epoch in range(200):
        ...
        for n in range(0, len(train), N):
            x = chainer.cuda.to_gpu(train[n:n + N].reshape((N, 1 * 28 * 28)))
            y, z = nn(x)
            # loss_reg = F.sum(y)
            # loss_rec = F.sum((x - z) ** 2)
            loss_reg = F.mean(y)
            loss_rec = F.sum((x - z) ** 2) / np.prod(x.shape)
            loss = loss_reg + lamda * loss_rec
            nn.cleargrads()
            loss.backward()
            optimizer.update()
